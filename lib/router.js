...
Router.route("/ids", {
  ...
  waitOn: function() {
    if (Meteor.user()) {
      return Meteor.subscribe("ownIdentificationsAndLinks", Meteor.user().profile.currentTraining);
    }
  },
  fastRender: true
});
...
Router.route("/pool", {
  ...
  waitOn: function() {
    if (Meteor.user()) {
      return Meteor.subscribe("otherIdentifications", Meteor.user().profile.currentTraining);
    }
  }
});
