...
Router.route("/ids", {
  ...
  waitOn: function() {
    if (Meteor.user()) {
      return Meteor.subscribe("myIdentificationsAndLinks", Meteor.user().profile.currentTraining);
    }
  }
  // fastRender: true
});
...
Router.route("/network", {
  ...
  waitOn: function() {
    if (Meteor.user()) {
      return [
        Meteor.subscribe("networkIdentifications", Meteor.user().profile.currentTraining),
