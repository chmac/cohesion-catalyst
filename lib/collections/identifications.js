...
Schemas.Identifications = new SimpleSchema({
  ...
  isPoolMatch: {
    type: Boolean,
    label: "Match selected in ID pool",
    optional: true
  },
  isInputMatch: {
    type: Boolean,
    label: "Match through input",
    optional: true
  },
  matchColor: {
    type: String,
...
Meteor.methods({
  ...
  /**
   * Meteor method for updating each document of the 'Identifications' collection
   * (and each of the associated documents in the 'Links' collection) which was not
   * created by the current user and which matches the passed text (i.e. name of the identification).
   * This method will update the respective documents with the current users '_id'
   * which gets added to a field 'matchedBy' (which is an array).
   * @param {string} idText - The name of the matched identification.
   * @param {Object} modifier - An oject that specifies how to update the document.
   */
  updateIdMatches: function(idText, modifier) {
    // We call 'this.unblock()' to allow the next method from this client to begin running.
    // this.unblock();
    ...
    Links.update({
      "source.createdBy": {$ne: currentUser._id},
    ...
    Links.update({
      "target.createdBy": {$ne: currentUser._id},
